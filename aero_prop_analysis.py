...
for V in range(V_stall, V_c_ft+1): 
    ...
    Ff_wing = (1+((0.6/xc_max)*tc)+(tc**4)) * (1.39*(M_wing**0.18)*math.cos(sweep_angle)**0.28)
    ...
    Ff_HT = (1+((0.6/xc_max)*tc)+(tc**4)) * (1.39*(M_wing**0.18)*math.cos(sweep_angle)**0.28)
    S_wet_HT = 2*S_HT
    ...
    Ff_VT = (1+((0.6/xc_max)*tc)+(tc**4)) * (1.39*(M_VT**0.18)*math.cos(sweep_angle)**0.28)
    S_wet_VT = 2*S_VT
    ...
    C_f_fuse = 0.455/((math.log10(Re_fuse)**2.58)*(1+(0.144*M_fuse**2))**0.65)
